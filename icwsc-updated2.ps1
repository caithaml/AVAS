...
$hash | Add-Member Noteproperty Startup (Get-CimInstance Win32_StartupCommand | Select-Object Name, command, Location, User)
...
$key                   = 'HKLM:SOFTWARE\Wow6432Node\Symantec\Symantec Endpoint Protection\CurrentVersion\SharedDefs' #cesta SEP 12.1 a vy���
...
$hash | ConvertTo-Json | Out-file -Path "D:/SICZ/avas/hash$(Get-Date -Format yyyy-MM-dd-hh-mm-ss).json" 