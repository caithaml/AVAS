Start-Transcript -Path "./transcriptGUI$(Get-Date -Format yyyy-MM-dd-hh-mm-ss).txt"
    ...
    if ($mElement.Name -eq $this.name)
    {
      foreach( $mProp in $mElement.Properties)
      {
        Switch($mProp.Name){ 
          ...
          'Top'
          {
            $mProp.Value = $this.Top
          } 
          'Left'
          {
            $mProp.Value = $this.Left
          } 
...
{
  $Global:mFormObj.Elements = $mFormObj.Elements | Where-Object -FilterScript {
    $_.Name -notlike $mFormObj.Elements[$mElemetnsGrid.CurrentRow.Index].Name
  } 
  renewGrids
} 
...
Function AddElement 
{
  ...
  $mSameType                = ($mFormObj.Elements | Where-Object -FilterScript {
      $_.Type -like $mControlType.SelectedItem
  }) 
...
function AddControl ($mControl) 
{
  ...
  switch ($mControl.Type){ 
    ...
    'TextBox'        
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.TextBox
    } 
    'ListBox'        
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.ListBox
    } 
    'ComboBoX'       
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.ComboBox
    }  
    'Label'          
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.Label
    }  
    'DataGrid'       
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.DataGridView
    }  
    'Button'         
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.Button
    }  
    'CheckBox'       
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.CheckBox
    } 
    'DateTimePicker' 
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.DateTimePicker
    } 
    'ListView'       
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.ListView
    } 
    'PictureBox'     
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.PictureBox
    } 
    'RichTextBox'    
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.RichTextBox
    } 
    'TreeView'       
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.TreeView
    } 
    'WebBrowser'     
    {
      $mReturnControl = New-Object -TypeName System.Windows.Forms.WebBrowser
    } 
    'default' 
    {
      Write-Host -Object 'neco se podelalo :('
    } 
  ...
  foreach ($mProperty in $mControl.Properties)
  {
    switch ($mProperty.Name){ 
      'Text'  
      {
        $mReturnControl.Text = $mProperty.Value
      } 
      'SizeX' 
      {
        $mSizeX = $mProperty.Value
      } 
      'SizeY' 
      {
        $mSizeY = $mProperty.Value
      } 
      'Top'   
      {
        $mReturnControl.Top = $mProperty.Value
      } 
      'Left'  
      {
        $mReturnControl.Left = $mProperty.Value
      } 
      'Anchor'
      {
        $mReturnControl.Anchor = $mProperty.Value
      } 
    }
  } 
 
  $mReturnControl.Size = New-Object -TypeName System.Drawing.Size -ArgumentList ($mSizeX, $mSizeY) 
  $mReturnControl.Add_MouseDown({
      mouseDown
  }) 
  $mReturnControl.Add_MouseMove({
      mouseMove -mControlName ($mControl.Name)
  }) 
  $mReturnControl.Add_MouseUP({
      mouseUP
  }) 
...
$mPropertiesGrid.Add_CellEndEdit({
    PropertiesEndEdit
}) 
